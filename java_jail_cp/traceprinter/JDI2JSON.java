...
public class JDI2JSON {
    ...
    private int last_step_line_stack_depth = 0;
    private int last_step_line_number = -1;
    private String last_step_line_method = null;
	...
	JsonValue returnValue = null;
	    ...
	    result.add("line", loc.lineNumber());
            ...
            int currentDepth = 0;
            try { currentDepth = t.frames().size(); } catch (Exception ex) {}
            // skip revisit of same line after returning from deeper calls
            if (loc.lineNumber() == last_step_line_number
                    && loc.method().name().equals(last_step_line_method)
                    && currentDepth < last_step_line_stack_depth) {
                return results;
            }
            last_step_line_number = loc.lineNumber();
            last_step_line_method = loc.method().name();
            last_step_line_stack_depth = currentDepth;
    ...
    public static String[] builtin_packages = {"java", "javax", "sun", "com.sun", "traceprinter", "jdk"};
    ...
    private JsonObjectBuilder convertFrame(StackFrame sf, boolean highlight, JsonValue returnValue) {
        JsonObjectBuilder result = Json.createObjectBuilder();
        JsonObjectBuilder result_types = Json.createObjectBuilder();
        ...
        catch (com.sun.jdi.OpaqueFrameException e) {
            JDWPerror = true; // JDK 9+ throws OpaqueFrameException instead of JDWP error 35
        }
		...
		if (lv.name().equals("args")) {
		    ...
		    if (v instanceof ArrayReference && ((ArrayReference)v).length()==0) continue;
                ...
                try {
                    ...
                    result_types.add(lv.name(), lv.typeName());
        ...
        boolean isConstructor = sf.location().method().name().equals("<init>");
        JsonObjectBuilder frameBuilder = Json.createObjectBuilder()
	    .add("func_name", (isConstructor ? sf.location().declaringType().name() : sf.location().method().name())+":"+sf.location().lineNumber())
	    .add("is_constructor", isConstructor)
	    .add("encoded_locals", result)
	    .add("local_types", result_types)
	    .add("ordered_varnames", result_ordered);
        if (returnValue != null && (showVoid || returnValue != convertVoid)) {
            frameBuilder.add("return_value", returnValue);
	}
	return frameBuilder
